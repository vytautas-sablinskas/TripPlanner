...
const EditBudget = () => {
  ...
  useEffect(() => {
    const fetchTravellers = async () => {
      ...
      const memberBudgets = data.budgetMembers.reduce(
        (acc: any, member: any) => {
      ...
      setSelectedMembers(
        data.budgetMembers.map((member: any) => ({
  ...
  const onSubmit = async (formValues: any) => {
    ...
    const response = await editTripBudget(
      getTripId(),
      getBudgetId(),
      actualValues
    );
                ...
                <FormItem className="mt-4">
                  ...
                  <FormControl className="create-edit-select">
                    <Input
                      disabled
                      value={
                        Object.values(BudgetTypes)[Number(selectedType)].value
                      }
                    />
                  </FormControl>
                  <FormMessage />
