...
export function TripTravellerList({
  ...
  const columns: ColumnDef<Payment>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        return (
          <div className="text-left font-medium">
            {getPermissionName(row.getValue("permissions"))}
          </div>
        );
      },
    },
    {
      ...
      cell: ({ row }) => {
        ...
        const handleDelete = async () => {
          ...
          const response = await deleteTripTraveller(
            getTripId(location),
            row.getValue("email")
          );
          if (!response.ok) {
        ...
        const handleEdit = async (permission: any) => {
          ...
          const response = await editTripTraveller(
            getTripId(location),
            data[row.index].id,
            permission
          );
          ...
          toast.success(
            `${row.getValue("fullName")} permissions were updated!`
          );
          setIsLoading(false);
        };
        ...
        return row.getValue("permissions") !== 2 && userPermissions === 2 ? (
          <DropdownMenu
            open={isMenuOpen}
  ...
  const [combinedFilterValue, setCombinedFilterValue] =
  ...
  const handleCombinedFilterChange = (
    ...
    const updatedFilters = [{ id: "email", value }];
      ...
      <div className="flex flex-col items-center py-4 sm:flex-row sm:items-end">
        ...
        <div className="sm:ml-auto mt-2 flex flex-col justify-end sm:items-end sm:flex-row w-full">
          {userPermissions === 2 && (
            <Button
              variant="outline"
              className="sm:mr-4 sm:w-auto w-full justify-start"
              onClick={() =>
                navigate(
                  Paths.TRIP_TRAVELLERS_CREATE.replace(
                    ":tripId",
                    getTripId(location)
                  )
                )
