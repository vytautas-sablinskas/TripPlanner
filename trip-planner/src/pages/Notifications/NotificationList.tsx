...
import { changeInvitationStatus } from "@/services/NotificationService";
...
export function NotificationList({ data, onStatusChange }: any) {
  const columns: ColumnDef<Notification>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="break-words overflow-auto max-w-[600px]">
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        const handleAccept = async () => {
          ...
          const response = await changeInvitationStatus(data[row.index].id, {
            status: 0,
          });
          if (!response.ok) {
  ...
  const handleCombinedFilterChange = (
    ...
    const updatedFilters = [{ id: "title", value }];
