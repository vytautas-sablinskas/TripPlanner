...
export const getRecommendationWeights = async () => {
    const token = localStorage.getItem('accessToken');
    const response = await fetch(ENDPOINTS.RECOMMENDATIONS.GET_RECOMMENDATION_WEIGHTS,
        {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
...
export const editRecommendationWeights = async (values : any) => {
    const token = localStorage.getItem('accessToken');
    const response = await fetch(ENDPOINTS.RECOMMENDATIONS.EDIT_RECOMMENDATION_WEIGHTS,
        {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
